...
(function ($) {
    function Movieseat(_paramObject) {
        ...
        var getMainLayoutFunc = function (_obj) {
            ...
            var _html = "<div class='seat-selection' style='width:" + (maxSeat + 1) * 45.5 + "px'>";
            for (var i = 0; i < _obj.objArea.length; i++) {
                _html += getAreaLayoutFunc(_obj.objArea[i]);
            }
            var actionPanel = pluginOptions.showActionButtons ? "<div class='seat-proccess-panel'><button type='button' class='layout-action-btn layout-btn-done " + pluginOptions.classes.doneBtn + "'> 決定 </button> </div>" : "";
        ...
        var getRowLayoutFunc = function (_obj, _area) {
            ...
            _html += "<li class='seat-row-seat row-indicator'>" + _obj.PhyRowId + "</li></ul>";
            return _html;
        }
    ...
    $.fn.seatLayout = function (_options) {
        ...
        _el.find('li').click(function (e) {
            if ($(this).hasClass('current-selected')) {
                var del = getObjData($(this));
                $(this).removeClass('current-selected');
                for (i = 0; i < selectedSeats.length; i++) {
                    if (selectedSeats[i]['PhyRowId'] == del['PhyRowId'] && selectedSeats[i]['seatNumber'] == del['seatNumber']) {
                        //spliceメソッドで要素を削除
                        selectedSeats.splice(i, 1);
                        _el.find('.layout-btn-done').prop('disabled', !(1 <= selectedSeats.length));
                    }
                }
            } else if ($(this).hasClass('can-select')) {
                ...
                selectedSeats.push(getObjData($(this)));
